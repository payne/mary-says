  ...
  User.sync({ force: true }) // We use 'force: true' in this example to drop the table users if it already exists, and create a new one. You'll most likely want to remove this setting in your own apps
    .then(function() {
      ...
      for (var i = 0; i < users.length; i++) {
        ...
        User.create({ firstName: users[i] }); // create a new entry in the users table
      }
    });
}
...
app.get("/says", function(request, response) {
  var dbUsers = [];
  User.findAll().then(function(users) {
    ...
    const n = Math.floor(Math.random() * dbUsers.length);
    const saying = dbUsers[n];
    response.send(saying); // sends dbUsers back to the page
  });
});

// creates a new entry in the users table with the submitted values
app.post("/says", function(request, response) {
  User.create({ firstName: request.query.fName });
